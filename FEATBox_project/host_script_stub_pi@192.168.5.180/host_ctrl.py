...
def send_database(func, address=None, command=None, cpuOrBoardName=None):
    feat_database = os.path.dirname(os.path.realpath(__file__)) + "/feat_database.json"
    ...
    if command == "status":
        ...
        if command in ["on", "off"]:
            ...
            col = "mode_status"
            if command == "uboot" or command == "reset" :
                valCol = False
            else:
                valCol = True
        ...
        def inner4():
            ...
            if ctrl == 0:
                ...
                # mycursor.execute("SELECT * FROM board_log order by id DESC limit 1")
                # board_id_log = mycursor.fetchone()[5]
                # mycursor.execute("SELECT * FROM board_log order by id DESC limit 1")

                # action_log = mycursor.fetchone()[3]
                # if board_id_log == board_id and action_log == command:
                #     mycursor.execute("UPDATE board_log SET status = True WHERE board_id = '" + board_id + "' AND action = '" + command + "' order by id DESC limit 1")
                # else:
                date = str(datetime.date.today())
